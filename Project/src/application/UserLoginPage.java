...
public class UserLoginPage {
	...
	private final SetupLoginSelectionPage setup_page;

	public UserLoginPage(DatabaseHelper databaseHelper, SetupLoginSelectionPage setup) {
	...
	public void show(Stage primaryStage) {
		...
		loginButton.setOnAction(a -> {
			...
			try {
				...
				int role = databaseHelper.getUserRole(userName);

				if (role != 0) {
					user.setRoleInt(role);
					if (databaseHelper.login(user)) {
						user.setID(databaseHelper.getUserUUID(userName));
